...
@implementation SLBSBrand

+ (NSArray *)brandsWithSources:(NSArray *)sources {
    if (sources && ([sources count] > 0)) {
        NSMutableArray *array = [NSMutableArray array];
        for (NSDictionary *source in sources) {
            SLBSBrand *entity = [SLBSBrand brandWithDictionary:source];
            if (entity) {
...
+ (SLBSBrand*)brandWithDictionary:(NSDictionary*)source {
    return [[SLBSBrand alloc] initWithDictionary:source];
}

- (instancetype)initWithDictionary:(NSDictionary *)source {
    self = [super init];
    if(self) {
        self.brandId =      [source objectForKey:@"id"];
...
- (id)initWithCoder:(NSCoder *)aDecoder{
    self = [super init];
    if(self) {
        self.brandId =      [aDecoder decodeObjectForKey:@"id"];
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
    [aCoder encodeObject:self.brandId forKey:@"id"];
